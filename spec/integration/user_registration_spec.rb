...
describe 'user registrations', type: :request do
  it 'is able to sign-up' do
    ...
    select "(GMT+02:00) Sofia", :from => "Your Timezone"
  ...
  it 'is not able to sign up with already registered email' do
    user = FactoryGirl.create :user, email: 'g@g.com'
    visit '/users/sign_up'
    within('body') do
      page.should have_content('Sign up')
    end
    ...
    fill_in "user[password]", :with=>"password"
    select "(GMT+02:00) Sofia", :from => "Timezone"
    click_button 'Sign up'
    within('body') do
  ...
  it 'is able to change their timezone when accepting invite' do
    user = FactoryGirl.create :user, invitation_token: '1234'
